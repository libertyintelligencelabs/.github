# Liberty Intelligence Labs: Code of Conduct

**Liberty Intelligence Labs** is a project-based collective where intelligence meets security through disciplined experimentation. As long as our experiments remain within the bounds of our safety and ethics framework, we encourage the bold, the experimental, and the unconventional. This Code of Conduct ensures that our efforts remain ethical, sustainable, and safe for our members and the college community.

## The Sandbox Rule
We play in a sandbox so the college doesn't have to.

* **Containment:** All high-risk cybersecurity experiments must be performed on isolated networks or virtualized environments.
  
* **No Harm:** Never test exploits or intrusive AI models on college infrastructure, faculty devices, or fellow students without explicit written authorization.
  
* **Resource Stewardship:** Lab assets—be they physical hardware, cloud compute credits, or shared API keys—are shared resources. Document your usage, respect rate limits, and never borrow components or credentials from another project's active setup.

## Radical Documentation
In this lab, a failed experiment is only a failure if it isn't documented.

* **The Legacy Requirement:** You are responsible for leaving a paper trail (code comments, READMEs, and schematics). Your tenure is not complete until your project is reproducible by the next cohort.

* **Fail openly:** Share your bugs. If you hit a wall, broadcast it. We value the data extracted from a mistake as much as a successful build.

## Ethical Stewardship
AI and cybersecurity are powerful tools. We use them to protect and innovate, not to surveil and harass.

* **Data Privacy:** Any data collected via IoT sensors must be anonymized. We do not build tools for nonconsensual tracking.

* **Responsible Disclosure:** If you discover a vulnerability in a system during your research, you must follow the lab's disclosure protocol—notifying the affected party before going public.

## The Collective Mindset
We are a project-based collectives, not a collection of rogue elephants.

* **Cross-Pollination:** A cybersecurity student should be willing to help an AIoT student secure their firmware, and vice versa.

* **Critique the System:** We value rigorous logic and kind delivery. Attack the code, the circuit, or the model—never the person.

## Project Life Cycle & Tenure
Participation is tied to your approved proposal.

* **Active Momentum:** Tenure is maintained through progress. If a project stalls significantly without communication, the lab reserves the right to reallocate resources as per the [Project Life Cycle SOP]().

* **Exit & Reentry:** Upon delivering your project's documentation and assets, your tenure concludes. To continue with the lab, a new proposal and safety review are required.

* **Ethical Breaches:** Any use of lab resources for malicious hacking or illegal activities result in immediate, permanent expulsion and a report to the CSIT Department.

---
We don't just study the future of secure and interconnected systems; we build it, one iteration at a time. By entering this lab, you agree to protect the sandbox, support the collective, and leave a legacy worth following.
